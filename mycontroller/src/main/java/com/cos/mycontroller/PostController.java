...
public class PostController {
	...
	// Get 방식은 데이터를 전송하는데에 목적이 있는것이 아니라 데이터 요구에 목적이 있음.
	...
	public User testPostObject(User user) { // x-www-form-urlencoded
		return user; // MessageConverter 낚아챔 -> json으로 변환 리턴(RestController 일때만)
		                      // MessageConverter 오브젝트 리턴시에만 발동
	}
	...
	public Integer testPath(@PathVariable int id) {
		return id;  // MessageConverter 낚아챔 -> json으로 변환 리턴(RestController 일때만)
							// MessageConverter 오브젝트 리턴시에만 발동
	}
}
